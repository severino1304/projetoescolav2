...
botao= pygame.image.load("images/botao.png")
botao1= pygame.image.load("images/botaopressionado.png")
...
def makeButton(x, y, width, height):
    entity = engine.Entity()
    entity.position = engine.Position(x, y, width, height)  # Posição e dimensões do botão
    # Animação para o botão (ativo e inativo)
    buttonInactiveAnimation = engine.Animation([botao])  # Frame inativo
    buttonActiveAnimation = engine.Animation([botao1])      # Frame ativo
    entity.animations = engine.Animations()
    entity.animations.add("inactive", buttonInactiveAnimation)
    entity.animations.add("active", buttonActiveAnimation)
    entity.type = "button"
    entity.activated = False  # Estado do botão
    entity.trigger = None     # Pode estar associado a algo (ex: plataforma)
    return entity

def makeMovingPlatform(x, y, width, height, speed, axis, distance):
    entity = engine.Entity()
    entity.position = engine.Position(x, y, width, height)
    entity.type = "moving_platform"
    entity.speed = speed
    entity.axis = axis  # 'x' para horizontal, 'y' para vertical
    entity.distance = distance
    entity.direction = 1  # Direção inicial
    entity.original_pos = (x, y)  # Guarda posição inicial
    entity.active = True  # Ativação pela lógica (ex: botão)
    return entity