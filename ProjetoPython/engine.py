...
class PhysicsSystem(System):
    ...
    def updateEntity(self, screen, inputStream, entity):
        ...
        for platform in globals.world.platforms:
            if isinstance(platform, pygame.Rect):
                if platform.colliderect(new_x_rect):
                    ...
                    break
            elif isinstance(platform, dict) and "rect" in platform:
                if platform["rect"].colliderect(new_x_rect):
        ...
        #... verificar contra todas as plataformas
        for platform in globals.world.platforms:
            # Se a plataforma for um pygame.Rect (plataforma estática)
            if isinstance(platform, pygame.Rect):
                if platform.colliderect(new_y_rect):
                    y_colisao = True  # Colisão
                    entity.speed = 0
                    # Se a plataforma estiver debaixo do jogador, ajustar posição
                    if platform.top > new_y_rect.y:
                        entity.position.rect.y = platform.top - entity.position.rect.height
                        entity.on_ground = True
                    break
            elif isinstance(platform, dict) and "rect" in platform:
                if platform["rect"].colliderect(new_y_rect):           
...
class LavaSystem:
    def __init__(self, alturainicial, lavaSpeed):
        self.alturainicial = alturainicial
        self.lavaSpeed = lavaSpeed
        self.type = "dangerous"
    def update(self):
        self.alturainicial -= self.lavaSpeed
    def draw(self, screen):
        lavaRect = pygame.Rect(0, self.alturainicial, 830, 830 - self.alturainicial)
        pygame.draw.rect(screen, (255, 0, 0), lavaRect)
    def checkCollision(self, entities):
        for entity in entities:
            if entity.type == "player" and entity.rect.bottom > self.alturainicial:
                return True
        return False
    
def updateMovingPlatform(platform):
    if not platform.active: # se estiver desativada, não mexe
        return
    if platform.axis == "x":
        platform.position.x += platform.speed * platform.direction # Inverte a direção se atingir o limite de movimento
        if abs(platform.position.x - platform.original_pos[0]) >= platform.distance:
            platform.direction *= -1
    elif platform.axis == "y":
        platform.position.y += platform.speed * platform.direction
        if abs(platform.position.y - platform.original_pos[1]) >= platform.distance:
            platform.direction *= -1
